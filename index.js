...
const BOT_PHONE_NUMBER = process.env.BOT_PHONE_NUMBER || "2348162084863";
...
// Reconnects back off (5s, 10s, 20s... capped at 60s) and are serialized,
// and a pairing code is only requested once per boot. This prevents a
// confirmed, widely-reported Baileys bug (statusCode 428 "Precondition
// Required" — requestPairingCode() racing a socket that's already closing
// during an instant reconnect) — but does NOT fix the separate, still-
// unexplained core issue where WhatsApp closes the connection shortly
// after issuing a pairing code, before linking completes. That part
